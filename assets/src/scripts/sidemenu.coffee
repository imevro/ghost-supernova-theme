...
SidebarMenuEffects = (->
  hasParentClass = (e, classname) ->
    ...
    return true  if e.classList.contains( classname )
  ...
  init = ->
    ...
    reset = document.getElementById("st-closeMenu")
    button = document.getElementById("st-showMenu")
    ...
    resetClickFn = (evt) ->
      if evt.target is reset
        resetMenu()
        document.removeEventListener eventtype, bodyClickFn

    button.addEventListener eventtype, (ev) ->
      ev.stopPropagation()
      ev.preventDefault()
      container.className = "st-container" # clear
      $( container ).addClass( button.getAttribute("data-effect") )
      setTimeout (->
